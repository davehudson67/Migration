...
MCMC_2 <- MCMCglmm(decline ~ migratory * Hemisphere +
                             migratory * EOO_log_centered +
                             migratory * (American + AfroPal + Asian) +
                             Hemisphere * EOO_log_centered +
                             Hemisphere * (American + AfroPal + Asian) + 
                             EOO_log_centered * (American + AfroPal + Asian),
...
# Export R workspace
save(MCMC_2, file = "MCMC_2.Rdata")
# Export model as object
saveRDS(MCMC_2, "MCMC_2.rds")
MCMC_2 <- readRDS("MCMC_2.rds")

# Export plots
dev.off()
par(mfrow=c(8,2), mar=c(2,2,1,0))
pdf(file = "MCMC_2.pdf")
plot(MCMC_2$Sol, auto.layout=F)
dev.off()
...
oneModel <- clean.MCMC(MCMC_2)  # get all the info from summary(modelName)
oneModel$modelName <- getName.MCMC(MCMC_2)  # add the model's name in a new column
oneModel  # check out the created dataframe

dataList <- list(MCMC_2)

dataListNames <- list("MCMC_2")
...
write.csv (mcmcOutputs, "MCMC_2.csv")
