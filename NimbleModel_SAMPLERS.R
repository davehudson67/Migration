...
conf  <- configureMCMC(Rmodel, monitors = c("beta","tau","sigma_animal"))
conf$removeSampler(c("animal_raw[1:500]", "tau", "beta"))
conf$addSampler(target = "animal_raw[1:500]", type = "AF_slice", control = list(adapt = TRUE))
conf$addSampler(target = "tau", type = "slice")
